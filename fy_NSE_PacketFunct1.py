...
## New code Ajay 2019-06-24
def process7202(logErr_inst, inputData, fyTokenDict, timestamp2_BCH, packetInfoDict, failedTokenDict, printFlag=False, debugFlag = False):
	"""
	[Function]  : 
	...
	try:
		...
		for everyRecord in range(0, numOfRecords):
			inputData = inputData[pointerIncri:] ## This is done at the begining since any continue statements make to loop on the same packet
			pointerIncri = NSE_P_7202_INF_SIZE
...
#				if token == 6109:
#					print oi
#				print "process7202: token: ",token
#				print "process7202: Market Type: ",mktType
				# print ' '.join(binascii.hexlify(inputData)[i:i+2] for i in xrange(0,len(binascii.hexlify(inputData)),2))
			except Exception,e:
				exc_type, exc_obj, exc_tb = sys.exc_info()
	...
	try:
		...
		while numOfPackets_NH > 0:
			...
			# 	dataDict[numOfPackets_NH + 1] = returnDict
				...
				returnDict = process7202(logErr_inst, dataPacket, fyTokenDict, timestamp2_BCH, packetInfoDict, failedTokenDict, printFlag, debugFlag)
				returnDict["tcode"] = transCode_BCH
				dataDict[numOfPackets_NH + 1] = returnDict
			else:
