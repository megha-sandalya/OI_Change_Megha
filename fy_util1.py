...
def setOiToThread(logErr_inst, cacheConn, inputExchange, inputSegment, inputDict, cacheQueue, sendPktQ, oiTokenDict, printFlag = False, debugFlag = False, nocache = False, noSend = False):
	"""
	[Function]   :   This function sends the oi dict to fyers packet function
	[Input]      :   logErr_inst  -> The instance of log_error which will log errors.
					 cacheConn    -> Contain the cache connection.
					 inputDict    -> This dictionary contain the processed data which is sent by NSE. Key in the dict will be token_number.
					 sendPktQ     -> The instance of send_packet to set the data to be send to streaming server to queue.
					 printFlag	  -> True/False = print/no_Print.
					 debugFlag 	  -> Contain the information that can used for debugging.
					 nocache 	  -> True/False used for debugging when set to True it will not take values from cache.
					 nosend       -> True/False used to send packets to streaming server when set to True will not send the packet to streaming server.
	[Output]     :   None
	"""
	try:
		for eachToken in inputDict:
			...
			curMinTS 		= timestamp_NSE - (timestamp_NSE % 60)
				...
				#				tokenOiDict = {}
#				if eachToken in oiTokenDict:
#					tokenOiDict[eachToken] = oiTokenDict[eachToken]

			if inputDict[eachToken]["oi"] < 0:
				print "Invalid OI"
				continue
#					if eachToken in tokenOiDict:
#						if "oi" in tokenOiDict[eachToken]:
#							if tokenOiDict[eachToken]["oi"] >= 0:
#								inputDict[eachToken]["oi"] = tokenOiDict[eachToken]["oi"]
...
#				print('\n')
			if nocache == False:
				cacheConn.set(memcToken_RT , inputDict[eachToken]["oi"] , ex=86400)
#				ohlcvData = json.loads(ohlcvData)
#				ohlcvData[fyMemC.K_MEMC_RT_OI] = inputDict[eachToken]["oi"]
#				print "setOiToThread: ",ohlcvData
#				cacheQueue.put({memcToken_RT:ohlcvData})
#				print('memcToken =========================== ' , memcToken_RT)
#				print('\n')
#				print('\n')
#				print('\n')
#				print('\n')
...
#				print('\n')
			...
			if noSend == False:
				sendDict = inputDict[eachToken]
				sendDict[fyMemC.K_MEMC_TRANS_CODE] = nseConf.NSE_T_CODE_7202
				sendDict[fyMemC.K_MEMC_TS_BCH] = inputDict[eachToken][fyCmnDef.K_BCH_TS]
				sendDict['tradeStat'] = 2 ## If the packet trans code is 7202 it is assumed that trading status is 2(open) 20190917 Palash
				sendPktQ.put({eachToken:sendDict})
	except Exception as e:
		exc_type, exc_obj, exc_tb = sys.exc_info()
		logErr_inst.LogError(None, "%s Line:%s. Unknow exception. Except:%s"%(fyCmnDef.ERROR_setOiPacket, exc_tb.tb_lineno, str(e)), printFlag)
